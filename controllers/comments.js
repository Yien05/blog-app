...
router.get("/", async (req, res) => {
    try {
        ...
        return res.status(400).json({ error: e.message, msg: "Cannot get all comment" });
    }
});
...
router.post("/:id", isAuth, async (req, res) => {
    try {
        ...
        return res.status(400).json({ msg: "Cannot add a comment, Please try again later", error: e.message });
    }
});

// update comment -> http://localhost:9999/comments/commentId
